...
def add_to_BS(graph, bit_string, node):
    bit = graph.nodes[node]['match_table'][node]
...
def remove_from_BS(graph, bit_string, node):
    bit = graph.nodes[node]['match_table'][node]
...
def failure(graph, node):
    ...
    for ie_node in ingress_egress:
        if ie_node == 0:
            match_table[ie_node] = 2**len(ingress_egress)
        else:
            match_table[ie_node] = 2**count
            count += 1

...
from Stateless_MC.Stateless_MC_logic import ingress_process, failure, print_list_SID, draw_graph, generate_random_dst

#------------------------- GRAPH DEFINITION --------------------------
from Stateless_MC.ISP1 import network as network_ISP1 # 650 nodes, 2300 edges
from Stateless_MC.rf1239 import network as network_rf1239 # 315 nodes, 1944 edges
from Stateless_MC.Cogentco import network as network_Cogentco # 197 nodes, 490 edges

from Stateless_MC.rf1239_copy import network as network_rf1239V2 # 315 nodes, 1944 edges
from Stateless_MC.Cogentco_copy import network as network_CogentcoV2 # 197 nodes, 490 edges

network_list = {"Cogentco": network_Cogentco, "rf1239": network_rf1239, "ISP1": network_ISP1}
network_listV2 = {"Cogentco": network_CogentcoV2, "rf1239": network_rf1239V2}
# --------------------------------------------------------------------
strategies = {"table": "Weight", "table_d": "Delay", "table_h": "\#Hop", "table_b": "Bandwidth", "table_phys": "Physical distance"}
strategiesV2 = {"table": "Weight", "table_d": "Delay", "table_h": "#Hop", "table_b": "Bandwidth", "table_phys": "Phys_dist"}
...
def protocol_measurements(perc, net_index, dest):
    e_num = len(dest)
    n_dest = len(dest[0][1])
    #print(
    #f"""
    #___________STATELESS MULTICAST____________
    ...
    #print()
    print(
f"""\multirow{{6}}{{*}}{{{n_dest} ({perc}\%)}}
& \multirow{{2}}{{*}}{{Stateless}} &\\vc Average &\\vc {round(measures["#node"]["Mean"], 5)} &\\vc {round(measures["#leaf"]["Mean"], 5)} &\\vc {round(measures["height"]["Mean"], 5)} &\\vc {round(measures["Av_B_Fact"]["Mean"], 5)}\\\ \cline{{3-7}}
...
def protocol_measurementsV2(file_name, exp, network_m, perc_dest, nbr_dest):
    ...
    for e in exp:
        ...
        height = tree_height(tree)
        av_b_f = tree_av_branch_fact2(tree, (n_node-n_leaf))
        f.write(f"{network_m} {perc_dest} {nbr_dest} Stateless {n_node} {n_leaf} {height} {av_b_f}\n")
    f.close()

# net_index = "Cogentco" or "rf1239"
# strategy = "table" or "table_d" or "table_h" or "table_b" (only for Cogentco: "table_phys")
def strategy_measurements(perc, net_index, dest, strategy):
    e_num = len(dest)
    n_dest = len(dest[0][1])
    #print(
    #f"""
    ...
    for e in dest:
        src = e[0]
        dst = e[1]
        tree = ingress_process(network, src, "512bytes", dst, return_tree=True, table=strategy)
        n_node = tree_size(tree)
        n_leaf = tree_n_leaf(tree)
        height = tree_height(tree)
        av_b_f = tree_av_branch_fact2(tree, (n_node-n_leaf))
    ...
    #print()
    print(
f"""& \multirow{{2}}{{*}}{{{strategies[strategy]}}} &\\vc Average &\\vc {round(measures["#node"]["Mean"], 5)} &\\vc {round(measures["#leaf"]["Mean"], 5)} &\\vc {round(measures["height"]["Mean"], 5)} &\\vc {round(measures["Av_B_Fact"]["Mean"], 5)}\\\ \cline{{3-7}}
&&\\bc Stand. Dev. &\\bc {round(measures["#node"]["Std"], 5)} &\\bc {round(measures["#leaf"]["Std"], 5)} &\\bc {round(measures["height"]["Std"], 5)} &\\bc {round(measures["Av_B_Fact"]["Std"], 5)}\\\ """, end=""
    )

# network_m = "Cogentco" or "rf1239"
# strategy = "table" or "table_d" or "table_h" or "table_b" (only for Cogentco: "table_phys")
def strategy_measurementsV2(file_name, exp, network_m, perc_dest, nbr_dest, strategy):
    f = open(file_name, "a")
    network = network_listV2[network_m]
    for e in exp:
        src = e[0]
        dst = e[1]
        tree = ingress_process(network, src, "payload", dst, return_tree=True, table=strategy)
        n_node = tree_size(tree)
        n_leaf = tree_n_leaf(tree)
        height = tree_height(tree)
        av_b_f = tree_av_branch_fact2(tree, (n_node-n_leaf))
        f.write(f"{network_m} {perc_dest} {nbr_dest} {strategiesV2[strategy]} {n_node} {n_leaf} {height} {av_b_f}\n")
    f.close()
